...
import { nextStep, prevStep } from "@/lib/store/slices/stepSlice";
import type {
  ProcessConfig,
  QualityParametersMatrixProps,
} from "@/interface/quality/quality-matrix.interfaces";
...
export function QualityParametersMatrix({
  ...
  const tooltipUtils = useMemo(
    () => ({
      shouldShowCalidadTooltip: (filaIdx: number) =>
        ...
        TextUtils.shouldShowTooltipCalidad(
          dataManagement.filas[filaIdx]?.[columnConfig.keyCalidad] ?? ""
        ),
      ...
      shouldShowHeaderTooltip: (col: string) =>
        ...
        TextUtils.shouldTruncateBasedOnSpace(
          col,
          columnConfig.columnasParams.length
        ),
    }),
  ...
  return (
    <div className="w-full flex flex-col h-full ">
      <div className="w-full h-12/15 overflow-y-auto">
        <div className="pb-4">
          <Title title="Parámetros de Calidad"></Title>
        </div>

        <div className="border-[3px] border-[#8bc34a] rounded-md overflow-hidden bg-white shadow-lg w-full">
            ...
            <div className="grid h-full" style={{ gridTemplateColumns }}>
              {showCheckboxes && (
                <div className="border-r-[3px] border-[#8bc34a] bg-white sticky left-0 z-20">
                  <div className="flex flex-col">
                    ...
                    <div
                      className={`${STYLES.HEADER} border-r-[3px] border-[#8bc34a]`}
                    ></div>
                    {dataManagement.filas.map((_, filaIdx) => (
                      ...
                      >
                        <Checkbox
                          ...
                          onChange={(e, data) =>
                            handleCheckboxChange(filaIdx, data.checked === true)
                          }
                          size="medium"
                        />
                      </div>
                  ...
                  {dataManagement.filas.map((fila, filaIdx) => (
                    <TooltipCell
                      ...
                      className={`border-b border-[#8bc34a] py-2 px-0.5 bg-white flex items-center justify-center text-xs font-medium h-10 min-h-10 overflow-hidden text-ellipsis whitespace-nowrap relative ${
                        showCheckboxes && selectedRowIndex === filaIdx
                          ? "bg-blue-50"
                          : ""
                      }`}
                    >
                      {TextUtils.truncateCalidad(
              ...
              <div className="overflow-x-auto overflow-y-hidden scrollbar-thin scrollbar-track-gray-100 scrollbar-thumb-[#8bc34a] hover:scrollbar-thumb-[#7cb342]">
                <div className="flex flex-col min-w-fit w-full">
                  {/* Fila de ajustes */}
                  <div
                    className={`grid gap-0 px-1 items-center min-w-fit w-full ${STYLES.ADJUSTMENT.replace(
                      "flex",
                      "grid"
                    )}`}
                    style={{
                      gridTemplateColumns: columnConfig.paramsGridColumns,
                    }}
                  >
                    {columnConfig.columnasParams.map((col, idx) => (
                      <div
                        ...
                        className={`flex justify-center items-center px-0.5 w-full min-w-[80px] border-r-2 border-[#8bc34a] h-full ${
                          idx === columnConfig.columnasParams.length - 1
                            ? "!border-r-0"
                            : ""
                        }`}
                      >
                        <AdjustmentInput
                          value={
                            adjustmentManagement.encabezadoSuperior[
                              idx
                            ]?.toString() ?? ""
                          }
                          index={idx}
                  ...
                  <div
                    className={`grid gap-0 min-w-fit w-full ${STYLES.HEADER.replace(
                      "flex",
                      "grid"
                    )}`}
                    style={{
                  ...
                  >
                    {columnConfig.columnasParams.map((col, idx) => (
                      <TooltipCell
                        ...
                        onMouseEnter={() =>
                          mouseHandlers.handleMouseEnter(-1, col, "header")
                        }
                        onMouseLeave={mouseHandlers.handleMouseLeave}
